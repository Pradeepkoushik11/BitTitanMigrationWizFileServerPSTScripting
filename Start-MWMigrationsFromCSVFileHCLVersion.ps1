...
(
    ...
    [ValidateScript( { $_ -in ('Calendar', 'Contact', 'Mail', 'Journal', 'Note', 'Task') } )] [String[]] $BitTitanMigrationItemType,
                ...
                if ([string]::IsNullOrEmpty($BitTitanProjectType)) {
                    ...
                    do {
                        ...
                        if (($result -match "^\d+$") -and ([int]$result -ge 0) -and ([int]$result -lt $script:connectors.Length)) {
                            ...
                            Write-Host -ForegroundColor Green "INFO: Selected ProjectId '$($script:connector.Id)'"
...
#######################################################################################################################"
        ...
        Write-Host -ForegroundColor Green "INFO: $($migrationsToSubmit.Count) migrations being submitted."
    }
    else {
...
#######################################################################################################################"
        ...
        Write-Host -ForegroundColor Green "INFO: $($migrationsToSubmit.Count) migrations being paused."
    }
    ...
    foreach ($mailbox in $migrationsToSubmit) {
        ...
        if ($statusAction -ne 4) {   
            try { 
                ...
                if ($mailbox.ExportType -eq "Pst") {   
                    $allPstMigrationsToTheSameMailbox = @(Get-MW_Mailbox -Ticket $script:mwTicket -ImportEmailAddress $mailbox.ImportEmailAddress | where { $_.ImportEmailAddress -eq $mailbox.ImportEmailAddress } )                 
                    $additionalPstMigrations = @( $allPstMigrationsToTheSameMailbox | where { $_.id -ne $mailbox.MailboxId } )
                    
                    $additionalPstMigrationsCount = $additionalPstMigrations.Count - $completedPstMigrationsCount 

                    if ($additionalPstMigrationsCount -ge 1) {
                        if (!$firstPST) {
                            $firstPST = $true

                            if (!$isPSTlabeled) {
                                $isPSTlabeled = $true
                                foreach ($pstMigrationToTheSameMailbox in $allPstMigrationsToTheSameMailbox) {
                                    $result = Set-MW_Mailbox -Ticket $script:mwTicket  -mailbox $pstMigrationToTheSameMailbox -Categories ";tag-5;"
                                }         
                            }                   

                            $blockPstSubmission = $false
                        }
                        else {
                            $blockPstSubmission = $true
                        }
                    }
                    else {
                        $blockPstSubmission = $false
                    ...
                    $additionalPstMigrationsCount = 0
                    $blockPstSubmission = $false
                }
        ...
        switch ($currentStatus) {
            ...
            "Completed" {            
                if ($mailbox.ExportType -eq "Pst") {  
                    $completedPstMigrationsCount += 1

                    $msg = "ERROR: Failed to submit migration '$($mailbox.ImportEmailAddress)' in '$projectName' --> Skipped because of PST migration completed'."
                    Write-Host -ForegroundColor Red  $msg
                    ...
                    $errorList | Add-Member -MemberType NoteProperty -Name Exception -Value "CompletedMultiplePSTMigration"
            ...
            "Failed" {      
                ...
                if ($mailbox.ExportType -eq "Pst" -and $blockPstSubmission) {  
                    $msg = "ERROR: Failed to submit migration '$($mailbox.ImportEmailAddress)' in '$projectName' --> Skipped because of multiple PST migrations to the same mailbox'."
                    Write-Host -ForegroundColor Red  $msg
                    ...
                    $errorList | Add-Member -MemberType NoteProperty -Name itemStartDate -Value $itemStartDate
                    $errorList | Add-Member -MemberType NoteProperty -Name itemEndDate -Value $itemEndDate
                    $errorList | Add-Member -MemberType NoteProperty -Name Exception -Value "MultiplePSTMigrations"
        ...
        if ($submit -and !$blockSubmission) { 
            ...
            elseif ($migrationType -eq [MigrationProxy.WebApi.MailboxQueueTypes]::Full -or $migrationType -eq [MigrationProxy.WebApi.MailboxQueueTypes]::Repair -or $migrationType -eq [MigrationProxy.WebApi.MailboxQueueTypes]::Trial) {
                ...
                if ($projectType -eq "Archive" -and !$quickSwitch) {
                    if(-not [string]::IsNullOrEmpty($BitTitanMigrationItemType)) {
                        $itemTypes = $($BitTitanMigrationItemType -join ",")
                    }
                    else {
                        $itemTypes = $null
                    }                    
...
#######################################################################################################################"
    ...
    if ($errorCount -gt 0) {
        do {
            try {
                $msg = "ERROR: $errorCount migrations failed to submit report exported to '$script:workingDir\FailedToSubmitReport-$script:date.csv'."
                Write-Host -ForegroundColor Red $msg
                Log-Write -Message $msg
        ...
        if (-not [string]::IsNullOrEmpty($BitTitanWorkgroupId) -and -not [string]::IsNullOrEmpty($BitTitanCustomerId)) {
            ...
            if (!$global:btCheckCustomerSelection) {
                do {
                    ...
                    Write-Host
                    $msg = "INFO: Selected Customer '$script:customerName' and OrganizationId '$global:btCustomerOrganizationId' / CustomerId '$global:btCustomerId'."
                    Write-Host -ForegroundColor Green $msg
                ...
                Write-Host
                $msg = "INFO: Already selected WorkgroupId '$global:btWorkgroupId' and OrganizationId '$global:btCustomerOrganizationId' / CustomerId '$global:btCustomerId'."
                Write-Host -ForegroundColor Green $msg
        ...
        do {
            ...
            if ($script:inputFile) {
                ...
                try {
                    ...
                    if($importedConnectors.Count -eq 0) {
                        $msg = "ERROR: No migrations found in '$ProjectsCSVFilePath'."
                        Write-Host -ForegroundColor Red  $msg
                        Log-Write -Message $msg  
                        Return
                    } 
